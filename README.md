...
-   [Snaplet](https://snaplet-seed.netlify.app/) for generating deterministic seeded data
...
-   Run `npx @snaplet/seed login` to log into Snaplet

-   Use `npm run post_checkout` to install any dependencies.

    -   If you get an error similar to > Stopped supabase local development setup.
...
_WARNING_: If you're recreating a view in your migration, make sure you create it `with(security_invoker = true)`, or the view is publicly available.
